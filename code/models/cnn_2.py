...
def convolution_mnist(x, n_ch, n_feature_maps, n_units, n_z, init, scope, reuse):
    """
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        x = linear(x, n_out=n_units, init=init, scope='linear_layer', reuse=reuse)
        x = nonlinearity(x)

        mu = linear(x, n_z, init=init, scope="mu_layer", reuse=reuse)

        sigma = linear(x, n_z, init=init, scope="sigma_layer", reuse=reuse)
        sigma = tf.nn.softplus(sigma)

        return mu, sigma


def deconvolution_mnist(z, n_ch, n_feature_maps, n_units, init, scope, reuse):
    """
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        z = linear(z, n_out=n_units, init=init, scope='mu_sigma_layer', reuse=reuse)
        z = nonlinearity(z)

        h = w = 3
        dim = h * w * n_feature_maps
        z = linear(z, n_out=dim, init=init, scope='linear_layer', reuse=reuse)
        z = nonlinearity(z)
...
def linear(x, n_out, init, scope, reuse):
    """
    Linear tranform
    """
    with tf.variable_scope(scope, reuse=reuse):
